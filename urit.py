...
class UritGenerator(Generator):
    ...
    def generate_barcode(self, item, ref, ed_r, bq):
        ...
        size = prs[0][3]
        item_id = prs[0][2]
        vol = '{:04}'.format(int(prs[0][0]) + int(prs[0][1]))
        ed = self.expiry_date(ed_r)
        bcs = [''] * bq
        for b in range(1, bq + 1):
            ...
            now = datetime.datetime(int(ed[-4:]),
                                    int(ed[2:4]),
                                    int(ed[:2]))
    ...
    def bn_gen():
        ...
        return '{:02}'.format(nowdate.day) + \
               '{:02}'.format(nowdate.month) + \
               '{:02}'.format(int(str(nowdate.year)[2:]))
